...
<div class="container-form-goods">
	<form id="goods-select" action="" method="get">
		<label>
			<select class="styledselect" name="author">
				<option <?php if(@$_SESSION['author'] == 'All authors') {echo 'selected';}?>>All authors</option>
				<?php while($row=$select1->fetch_assoc()) { ?>
					<option value="<?php echo $row['id'] ?>" <?php if(@$_SESSION['author'] == $row['id']) echo 'selected' ?>><?php echo $row['author']; ?></option>
				<?php } ?>
			</select>
		</label>
		<label>
			<select class="styledselect" name="genre">
				<option <?php if(@$_SESSION['genre'] == 'All genres') {echo 'selected';}?>>All genres</option>
				<?php while($row=$select2->fetch_assoc()) { ?>
					<option value="<?php echo $row['id'] ?>" <?php if(@$_SESSION['genre'] == $row['id']) echo 'selected' ?>><?php echo $row['genre']; ?></option>
				<?php } ?>
			</select>
		</label>
	...
	<form id="goods-button" action="" method="get">
		<button class="add-button" type="submit"><a href="/admin/books/add">Добавить</a></button>
							...
							<div id="table-content">
								...
								<form id="mainform" action="">
									<table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
										...
										<?php if(isset($_SESSION['author'], $_SESSION['genre'])) { for($i = 0;$row=$res->fetch_assoc();$i++) {
											...
											<tr class="<?php echo $class ?>">
												<td class="options-width">
													...
													<a href="/admin/books/edit/?id=<?php echo $row['id'] ?>" title="Edit" class="icon-1 info-tooltip"></a>
													<a href="/admin/books/home/?delete=<?php echo $row['id'] ?>" title="Delete" class="icon-2 info-tooltip"></a>
												</td>
